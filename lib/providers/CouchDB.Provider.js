...
CouchDB.prototype.findByKey = function (key, callback, rev) {
	...
	process.success(function (update, body) {
		if (body.rows.length === 0) {
			if (rev) {
				throw new Error("Item with key '" + key + "' and rev '" + rev + "' does not exist");
			}
		...
		if (body.rows.length > 1) {
			if (rev) {
				throw new Error("Found multiple items with key '" + key + "' and rev '" + rev + "'");
			}
			throw new Error("Found multiple items with key '" + key + "'");
		}

		if (body.rows[0].error) {
			if (rev) {
				throw new Error("Item with key '" + key + "' and rev '" + rev + "' does not exist");
			}
			throw new Error("Item with key '" + key + "' does not exist");
		...
		if (body.rows[0].error) {
			throw new Error(body.rows[0].error);
		}

		return update(JSON.parse(JSON.stringify(body.rows[0].doc)));
	});

	this.__provider.fetch({ keys: [key], include_docs: true }, process.complete);
...
CouchDB.prototype.findManyByKey = function (key, callback, rev) {
	...
	process.success(function (update, body) {
		if (body.rows.length && body.rows[0].error && body.rows[0].error !== "not_found") {
			if (rev) {
				throw new Error("Item with key '" + key + "' and rev '" + rev + "' returned the following error '" + body.rows[0].error + "'");
			}
			throw new Error("Item with key '" + key + "' returned the following error '" + body.rows[0].error + "'");
		}
		if (body.rows.length && body.rows[0].error === "not_found") {
			body.rows = [];
		}

...
function MongoDB(config) { 
	 ...
	 * @param callback
	 */
	this.getByKey = function getById(filter, callback) {
...
//	};
	...
	/**
	 * Update method heps updating specific fields of the document. It has a built in
	 ...
	 * If no query has been passed by, the search without a filter, find all documents.
	 * @param filter
	 * @param callback
	 */
	this.getAllWithFilter = function getAll(filter, callback) {
		if (!filter) { self.getAll(callback); }
